...
def test_all_my_bookings(db_connection):
    ...
    booking_repo.create(Booking(None, 1, 2, date(2024, 9, 1), date(2024, 9, 4), False, 300.0, date(2024, 7, 9)))

    assert mybookingrepo.all_my_bookings(2) == [
        MyBooking('Studio in London', 1, 'Will Test', 2, False, 300.0, date(2024, 9, 1), date(2024, 9, 4))
    ]
...
def test_all_my_property_bookings(db_connection):
    ...
    assert mybookingrepo.all_my_property_bookings(1) == [
        MyBooking('Studio in London', 1, 'Marco Polo', 1, False, 360, date(2024, 7, 5), date(2024, 7, 11)),
        MyBooking('Studio in London', 1, 'Will Test', 2, False, 300.0, date(2024, 9, 1), date(2024, 9, 4))
    ]

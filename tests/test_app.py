...
def test_property_list_route(client):
    ...
    response = client.get('/p_list', query_string={'spaces': json.dumps(spaces)})
    assert response.status_code == 200
...
def test_property_detail_route(client):
    ...
    response = client.get('/property/1', query_string={'spaces': json.dumps(spaces), 'owners': json.dumps(owners)})
    assert response.status_code == 200
    assert b'221b Baker Street' in response.data
    assert b'Martha Hudson' in response.data

    response = client.get('/property/999', query_string={'spaces': json.dumps(spaces), 'owners': json.dumps(owners)})
